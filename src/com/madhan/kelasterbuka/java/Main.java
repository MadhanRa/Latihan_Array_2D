package com.madhan.kelasterbuka.java;

public class Main {
    public static void main(String[] args) {
        int[][] matrix_a = {
                {1,2,3},
                {4,5,6},
                {7,8,9}
        };
        int[][] matrix_b = {
                {11,12,13},
                {14,15,16},
                {17,18,19}
        };
        printArray(tambah(matrix_a,matrix_b));
        int[][] matrix_c = {
                {1,2},
                {3,4}
        };
        int[][] matrix_d = {
                {11,12},
                {13,14}
        };

        printArray(kali(matrix_c,matrix_d));


    }

    private static int[][] kali(int[][] matrix_1, int[][] matrix_2) {
        int baris_1 = matrix_1.length;
        int kolom_1 = matrix_1[0].length;
        int baris_2 = matrix_2.length;
        int kolom_2 = matrix_2[0].length;
        int[][] hasil = new int[baris_1][kolom_2];
        if (kolom_1 == baris_2) {
            int buffer;
            for (int i = 0; i < baris_1; i++) {
                for (int j = 0; j < kolom_2; j++) {
                    buffer = 0;
                    for (int k = 0; k < kolom_1; k++) {
                        buffer += matrix_1[i][k] * matrix_2[k][j];
                    }
                    hasil[i][j] = buffer;
                }
            }
        } else {
            System.out.println("Jumlah kolom matrix_1 tidak sama dengan jumlah baris matrix_2");
        }
        return hasil;
    }

    private static int[][] tambah(int[][] matrix_1, int[][] matrix_2) {

        int baris_1 = matrix_1.length;
        int kolom_1 = matrix_1[0].length;
        int baris_2 = matrix_2.length;
        int kolom_2 = matrix_2[0].length;
        int[][] matrixHasil = new int[baris_2][kolom_1];

        if (baris_1 == baris_2 && kolom_1 == kolom_2) {
            for (int i = 0; i < baris_1; i++) {
                for (int j = 0; j < kolom_1; j++) {
                    matrixHasil[i][j] = matrix_1[i][j] + matrix_2[i][j];
                }
            }
        } else {
            System.out.println("Jumlah baris dan kolom tidak sama");
        }

        return matrixHasil;
    }

    public static void printArray(int[][] dataArray) {
        int baris = dataArray.length;
        int kolom = dataArray[0].length;

        for (int i = 0; i < baris; i++) {
            System.out.print("[");
            for (int j = 0; j < kolom; j++) {
                System.out.print(dataArray[i][j]);

                if (j < (kolom - 1)) {
                    System.out.print(",");
                } else {
                    System.out.print("]");
                }
            }
            System.out.println();
        }
        System.out.println();
    }
}


